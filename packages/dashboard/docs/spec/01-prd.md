...
```yaml
name: "Daily cross-ref"
personas: [PERSONA-1]
triggers: ["DELTA proposal 작성 중 downstream 영향 확인"]
outcome: "5단계 ref 사슬을 클릭으로 추적, ~15초"
status: Approved
priority: P0
```
<!-- /specrail:attrs -->
...
personas: [PERSONA-1]
triggers: ["새로 추가한 NFR/AC 의 의미 품질 검증 필요"]
outcome: "AI 가 patch 제안 stream, 사용자 accept → atomic write + git diff"
status: Approved
priority: P0
```
<!-- /specrail:attrs -->
...
name: "Graph 탐색"
personas: [PERSONA-1]
triggers: ["새 feature 추가 후 downstream 누락(orphan/dangling) 점검"]
outcome: "1-hop filter + orphan toggle 로 누락 즉시 발견"
status: Approved
priority: P0
```
<!-- /specrail:attrs -->
...
target: 60
unit: percent
measure-when: "본인 daily usage 8주 후"
note: "baseline = 현재 spec 검토 1 session 15-25분"
```
<!-- /specrail:attrs -->
...
target: 0
unit: count
measure-when: "release 후 첫 4주"
note: "critical KPI — 위반 시 즉시 hotfix"
```
<!-- /specrail:attrs -->
...
target: 80
unit: percent
measure-when: "본인 sample 30 issues 후"
```
<!-- /specrail:attrs -->
...
target: 3
unit: count
measure-when: "v0.1 release 후 12주"
```
<!-- /specrail:attrs -->
...
target: 2
unit: second
measure-when: "phase view 500 ID, graph 200 노드"
```
<!-- /specrail:attrs -->
