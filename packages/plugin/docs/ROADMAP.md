...
**Date:** 2026-05-13
**From:** v0.0.1-alpha (logic 완성, wire-up 미완)
**To:** v0.1.0 (operational, alpha-validated)
...
**현재 logic을 실 사용자 commit에서 자동 작동하게 wire-up하고, 외부 통합 (CC marketplace·Plausible·alpha 사용자)으로 ship 가능한 plugin 완성.**

---

## Status quo (현재 alpha 시점)
...
현재와 동일 — Boil the Lake (단일 product 한 cycle). Dashboard·multi-LLM·Refactor 모두 deferred.
...
| **T5.2 Hook stub → live hook 교체** | `.git/hooks/pre-commit`을 alpha stub에서 `HOOK_TEMPLATE` 실 install. Self-install command. 본 repo는 dogfood로 적용 | 환각 ID 포함 commit이 차단됨 (exit 1) | 45-60min |
...
**M5 누적: 4-6h. Wire-up 완료 = plugin이 "logic 가진 stub"에서 "operational plugin"으로 전환.**
...
| Task | 내용 | 시간 |
|---|---|---|
| **T6.1 HOOK_TEMPLATE dist 미빌드 시 hard fail** | dynamic import 실패 시 `exit 1` (silent pass 제거). 안내 message + `npm run build` 가이드 | 20min |
...
| **T6.6 frontmatter manual edit detection (선택)** | `status: Approved`를 사용자가 수동 편집 시 — 이전 hook 통과 hash 검증 또는 signed approval marker. 본 task는 P1 (향후 후보) | 60min (P1) |
...
본 conversation의 가장 큰 lesson — plan self-check이 76 findings 못 잡음. 향후 자동화.
...
**M9 누적: 3-4h. 향후 plan self-check이 76 findings 패턴을 자동 잡음.**
...
| **T11.5 anti-sycophancy lint cleanup** | docs/ROADMAP.md·README.md·docs/spec/examples/*에서 lint:plan anti-sycophancy 41 violations cleanup. evidence 추가 또는 키워드 수정으로 PASS state 달성. (architect M9 condition C1 lock-in) | 30-60min |
...
| Frontmatter manual edit | signed approval (향후) | best-effort detection (현재) |
...
4. M1·M2 mega-commit 패턴 — 초기 plan TDD enforcement 미달성
...
본 plan = 이전 plan과 동일 process로 작성됐어야:
1. PRD (vision) — 본 plan §"한 줄 정의"
2. Personas — alpha 사용자 + 기존 사용자
3. Features — M5~M11 (45 atomic task)
4. Domain Model — 기존 ENT 그대로 + ENT-Orchestrator·ENT-Approval 추가
5-7. UX — CLI 명령 추가 (`approve`·`build`·`check:*`)
8. Architecture — Phase 8 ARCH-8~12 추가 (M11)
9. NFR — 기존 NFR + alpha 사용자 feedback metric
10. Test — M10
11. Operations — M8 (real endpoint·marketplace)
12. ADR + Risks — M7 결정 + 5건 silent failure ADR
13. Implementation — 본 plan 자체

즉 본 plan을 specrail plugin으로 사양화 가능. 본 plan의 dogfood evidence.

---

## 결론

본 plan = alpha → operational 전환 roadmap. 6 milestone, 45 atomic task, 28-38h AI 보조 + 외부 통합 별 cycle.

가장 critical: **M5 wire-up 4-6h가 plugin의 "logic"을 "operational"로 전환하는 핵심 단계.** 이 단계 후 본 repo의 hook이 진짜 작동 + ID auto-gen이 진짜 호출 + approve가 진짜 frontmatter write. alpha의 70% wire-up gap이 30%로 축소.

v0.1.0 ship 정의 = 11개 checklist. M5~M11 + alpha 사용자 1명 = ship 가능.

본 plan은 `docs/ROADMAP.md`. 다음 cycle 시작 시 input.
