# Review Findings — 4 Rounds Cumulative

본 문서는 4 라운드 reviewer agent 검토에서 발견된 76건 finding의 처리 결과 기록.
plan input + dogfood evidence 보존용.
...
| 9 High | H1 r7-b2b regex / H2 downstream BFS / H3 mock 100% / H4 EDGE 24/25 / H5 TC 28/65 / H6 AC 6/24 인용 없음 / H7 OQ-13 미해결 / H8 queue race / H9 IdCounter mutex | **H1·H6·H8·H9 fix (Batch 2·4) / H2·H7 annotation 추가 / H3·H4·H5 archived as future cycle needs** |
...
| 7 High | D4 secret 패턴 누락 / D5 path traversal / D6 flushQueue bypass / D7 ARCH 미등재 / D8 Phase N+1 auto-invoke / D9 S2 DELTA 후반부 / D10 graph.json stale | **D4·D5·D6 fix (Batch 1·3) / D7·D8·D9·D10 archived as future cycle spec update** |
...
| Batch | 처리 항목 | Test 결과 |
|---|---|---|
| Closure C1-C8 | LICENSE, status command, git tag archive, schema, review prompt, hook self-install, README 정정 | 249/249 PASS |
...
## Archived (향후 cycle input)

이미 commit된 사항 또는 본 conversation 범위 외 — 향후 task로 반영.

### Critical archived

- **C1 (Round 3)** TDD 5-step 위반 — M1·M2가 mega-commit. git history 변경 불가. 향후 hook으로 atomic commit 강제 (commit당 1 file change 등)
- **C3 (Round 3)** A1 PASSED-PARTIAL R8 진행 — manifest inject 미검증. 실 CC plugin install 후 1차 검증 필요. 향후 task: external integration milestone (M5)
- **C4 (Round 3)** 외부 통합 0 — Plausible/CC marketplace/실 사용자. 향후 M5

### High archived

- **H3 (Round 3)** 100% mock test — e2e도 내부 module 직접 호출. 향후: real CC harness mock (Playwright-like)
- **H4 (Round 3)** EDGE 24/25 미구현 — 시간/TZ·동시성·i18n·auth bypass·hook 무결성 test 추가. 향후 polish
- **H5 (Round 3)** TC 28/65 미구현 — 외부 의존 TC 제외하면 ~10 TC 추가 가능
- **H7 (Round 3)** OQ-13-1·2·3·5 미해결 — 출시 후 telemetry baseline 측정 후 결정
...
- **D9 (Round 4)** S2 DELTA 후반부 (delta skill invoke·merge·archive) 미구현 — 향후 R4 보강
- **D10 (Round 4)** graph.json cache 명시 stale — ADR-9 옵션 D 채택 후 spec doc 갱신

### Medium/Low archived

- D13 13 schema 통합 (common 1개 + runtime check) — 향후 simplification
...
## Plan self-check의 구조적 약점 (향후 plan 개선 후보)
...
- lessons learned doc input (본 REVIEW-FINDINGS.md)

---

## 본 conversation의 dogfood evidence

본 작업 전체가 plugin이 자동 강제해야 할 anti-pattern의 실 사례:
...
이 5 패턴 = plugin이 자동 lint해야 할 정확한 영역. 향후 lint extension 후보.
...
archived: 18 high/critical + 12 medium/low.

본 finding 기록은 향후 plan의 input. plan self-check enhancement + M5 external integration이 핵심 추가.
