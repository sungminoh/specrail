...
| **T5.5 `/plan-pipeline approve` CLI command** | frontmatter `status: Draft → Approved` write. 사용자 명시 trigger. `src/cli/approve.ts` 신규 | approve 후 transition gate unlock | 30-45min |
...
| **T7.5 S2 DELTA — archive 이동** | merge 완료 후 `changes/{date}-{topic}` → `changes/archive/` 이동. SM-Change: Applied → Archived | 30min |
...
| Task | 내용 | 시간 |
|---|---|---|
| **T8.1 npm publish workflow + scoped package** | `@plan-pipeline/v4` 또는 `@plan-pipeline/v0` scope 결정. `npm publish --access public` first run | 45min |
...
| "v0.1.0 ship-ready" | M8 alpha 사용자 단계 후 ship 가능. 본 plan 완료 = ship "candidate" |
