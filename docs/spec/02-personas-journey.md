...
- **사는 곳 / 사용 환경:** 데스크톱 작업 (집·사무실·카페) + markdown 검토 (GitHub UI·VS Code preview) + 모바일 메모 (이동 중 idea 수집). 인터랙티브 dashboard는 향후 cycle.
...
- **이미 시도한 대안:** 기존 markdown prompt 직접 사용, BMAD, OpenSpec, Cursor Rules, Claude Project Knowledge, Notion template
...
|---|---|---|---|---|---|
| 1 | Plugin 발견 | GitHub README · Twitter 보고 install | "기존 markdown prompt 써봤는데 plugin이 더 나을까" | curious | PAIN-base |
...
| 5 | Phase 1 skill 호출 | plugin 자동 호출 → 6 forcing questions | "흐름은 매끄러운가" | challenged | - |
...
| 9 | Markdown 검토 | GitHub UI 또는 VS Code preview에서 PRD + dependency graph mermaid 확인 | "전체 한 번 — 인터랙티브 검토는 향후에서" | satisfied | PAIN-4 Partial |
...
### Critical Step
- **Make-or-break:** Step 6 (Forcing 압박). 여기서 도망치면 PRD 약함이 후속 phase 전염.
- **Magic moment:** Step 8 (Hook 검증 자동 pass) + Step 11 (다음 phase 자동 ID 인계). plugin 가치 입증 지점.
...
(P1 — 초기 release scope 외. 명세 잠정. 향후 cycle에서 정식 Journey Map.)

## 6. Pain Priority

| Pain ID | 설명 | 빈도 | 영향도 | 우선 | 어느 시나리오 |
|---|---|---|---|---|---|
| PAIN-1 | 환각 ID (정의 안 된 ID 인용) | 기존 markdown 사용자 전원 | 높음 | P0 | S1, S2 |
...
| PAIN-fundamental | Forcing question 도망 | ~40% | 치명 | P0 | S1 |
| PAIN-6 | 다중 project 동시 검토 부담 | 30% (multi-project user) | 중 | P1 | (Edge-2) |
| PAIN-7 | Brownfield 역설계 부담 | brownfield user 전원 | 매우 높음 | P1 | S3 (향후) |
| PAIN-base | Plugin install·setup 마찰 | 첫 사용자 전원 | 중 | P0 | S1 |

## 7. 차단 단계 (Blocking Step)

- **S1 차단:** Step 6 (Forcing 압박에서 도망) → PAIN-fundamental 해결 필수.
...
|---|---|---|---|
| OQ-2-1 | Edge-1 (non-developer)을 초기 release primary 승격? Phase 3 결정 | maintainer | N |
| OQ-2-2 | Edge-2 (multi-project) 대시보드 multi-tab 초기 release 포함? | maintainer | Phase 6 IA |
| OQ-2-3 | S3 Refactor — 향후 cycle 일정 | maintainer | Phase 13 |
