...
v4 Plugin v4.0 — Claude Code skill collection (13 phase + orchestrator + telemetry skill) + hook scripts + builders. 사용자가 `/plan-pipeline` 한 명령으로 spec 사양화 시작, 13 phase HARD-GATE를 도구로 강제, KPI-2 환각 ID 0 보장 ship.
...
| ID gen | Sequential counter (`.plan-pipeline-cache/id-counter.json`) | ADR-5 |
...
|---|---|---|---|
| S1 Greenfield (`/plan-pipeline` → 13 phase) | R1·R2·R5·R6·R7·R8·R13 | M1+M2+M3 | ✅ |
...
`package.json`: name `@plan-pipeline/v4`, version `0.0.1`, type `module`, engines `>=20`, scripts (test/typecheck), devDeps (vitest, typescript, @types/node), deps (ajv, unified, remark-parse, remark-frontmatter).

`tsconfig.json`: target ES2022, module ESNext, strict, resolveJsonModule, skipLibCheck, outDir dist.

`.gitignore`: node_modules/, dist/, .plan-pipeline-cache/, *.log.
...
- static `load(projectRoot)`: readFile `.plan-pipeline-cache/id-counter.json`, on miss return EMPTY clone
...
- `syncCache(projectRoot)`: optional — frontmatter ↔ `.plan-pipeline-cache/state.json` 비교, 다르면 frontmatter 기준으로 cache 재생성. Hook이 commit 시 호출.
...
5. Husky·lefthook 감지 시: 그들의 chain 메커니즘 활용 (자체 hook 추가하지 말고 husky config에 `npx plan-pipeline check` 라인 추가 권장)
...
Condensed (옵션 A 시): `src/graph/cache.ts`. RED: cache 없으면 매 hook full rebuild → NFR-PERF-3 위반 위험. GREEN: changed file modtime 비교 + diff parse + cache merge. Commit: `feat(graph): incremental rebuild + plan-pipeline-cache (ADR-9 옵션 A, NFR-PERF-5, TC-74)`
...
Files: `src/skill/change.ts`. RED: change 명령 → proposal 부재. GREEN: T2.3로 영향 phase 추출 + `changes/{date}-{topic}/proposal.md` template 작성. Commit: `feat(skill): plan-pipeline change auto-draft (F4.3, AC-R4-1)`
...
GREEN:
1. Install: `claude code skill install @plan-pipeline/v4` 또는 GitHub install 명령
2. Usage: `/plan-pipeline init` → Phase 1 시작 + 13 phase 진행 flow
...
Files: `src/telemetry/consent.ts`. RED: install 시 default opt-in (privacy 위반). GREEN: 명시 yes/no 질문 + default OptedOut + `~/.plan-pipeline/consent.json`. Commit: `feat(telemetry): install opt-in default OptedOut (F13.1, INV-9, AC-R13-1, TC-21·38)`
...
Condensed: `src/cli/opt-out.ts`. RED: opt-out 명령 부재. GREEN: consent.json status=OptedOut + 데이터 삭제 요청 mailto 안내. Commit: `feat(cli): plan-pipeline opt-out command (F13.3, AC-R13-3, TC-23)`
...
- S2: 기존 S1 spec 보유 → `/plan-pipeline change "add payment"` → 영향 phase 자동 식별 (Phase 1·3·4·8·12·13) → ADDED/MODIFIED/REMOVED proposal draft → 사용자 승인 → delta 작성 → current/ 머지 → archive 이동.
