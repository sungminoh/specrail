...
**Reference:** `/reference-v3/12-adr-risks.md`
**Date:** 2026-05-12
...
- **단점:** Claude Code 외 fallback 어려움 (v3 markdown은 별도 유지 필요)
...
- **거절 이유:** PRD §6 Non-Goal "Claude Code 외 다른 agent tooling — v3 markdown으로 fallback" 명시. 자체 loader 구현은 maintainer 시간 낭비 (Boring by default)
...
**Per-phase per-project sequential counter.** `.plan-pipeline-cache/id-counter.json`에 phase별 다음 번호 저장. Counter는 git ignore (rebuild 가능 — graph builder가 max(used) + 1).

ID 형식: phase 마다 prefix (R{n}, F{n}.{m}, S{n}.{m}.{k}, ENT-{Name}, INV-{n}, NFR-{Domain}-{n}, ARCH-{n}, EXT-{n}, OPS-{n}, ADR-{n}, RISK-{n}, T{n}.{m}). v3 README §126-150 그대로 차용.
...
##### 긍정
- v3 README §126-150 그대로 차용 (도메인 무관성 R7 일관)
- 사용자가 ID grep 가능 (`grep -r 'S1.2.3' docs/spec/`)

##### 부정 (수용 가능)
- ID 중간 삭제 시 hole 발생 (R3가 deferred돼도 R3 reservation — 단 v4에서 R3는 명시 deferred로 처리됨, 새 R 부여 시 max+1)

##### 영향받는 phase·ARCH
- ARCH-6 (ID Auto-gen): counter 관리
- ARCH-4 (Graph Builder): rebuild 시 max+1
- Phase 13: counter init task (`.plan-pipeline-cache/id-counter.json`)

#### Trigger to Re-evaluate

- NFR-SCAL-4 가정 변경 (multi-user 동시 — v4.5 dashboard cycle에서)
- 5000 ID 한계 도달 (NFR-SCAL-2)

#### References

- Phase 8 §9 ADR-CAND-5
- v3 README §126-150 ID 체계
- Phase 3 R1, F1.3
...
##### 부정 (수용 가능)
- A1 spike 미통과 시 v4.0 release 지연 (RISK-2 mitigation: Phase 13 M0 spike task)
...
**ADR-6b Decision:** **R8 implementation scope를 v4.0에서 축소.** Phase 13 후 Implementation 핸드오프는 **사용자 수동 + Claude Code main session** (현재 Claude Code 표준 흐름). Subagent fresh context guarantee 없이도 사용자가 task별 ctrl-N 새 chat 시작으로 부분 대체.

**Affected tasks (Phase 13):**
- T3.4 (subagent wrapper) → **DEFER to v4.1**. 대신 README에 "Phase 13 task별 새 Claude Code chat 시작 권장" 가이드.
- T3.5 (2-stage review) → **DEFER to v4.1**. M3에서 제외.
- T3.6 (BLOCKED escalation) → **DEFER to v4.1**. 사용자가 직접 막힘 인식.
- F8.1~F8.4 → **Status: Deferred (v4.1)** — Phase 3 update DELTA.
- R8 Importance: P0 → P1 (v4.1 후보).
- KPI-1 영향: 완주율 측정 시 Phase 13 task 진행 metric 별도 — telemetry로 PhaseApproved(13) 카운트만.

**Trigger to revisit:** CC SDK가 subagent 기능 정식 공개 또는 third-party (Aider 등) 표준화 시 → v4.1 R8 부활.
...
- **Derived (cache):** `.plan-pipeline-cache/state.json` — frontmatter에서 build됨. Skill·hook이 빠른 lookup용.
...
- 사용자 10건 이상이 "phase 진행 자유" 요구 — 단 그 경우는 v3 markdown fallback
...
**Innovation token:** No (v3 README §174-196 차용)

#### Context

각 phase 산출물 file system layout 결정. v3 README §174-196 패턴은 phase 7 (wireframe), 12 (ADR-risks)는 디렉토리, 나머지는 단일 파일. v4도 동일 적용 vs 모두 통일?
...
**Single file default + 디렉토리 (phase 7, 12, 13).** v3 README §174-196 패턴 그대로:
...
##### 옵션 A (선택됨): Single + directory mixed (v3 패턴 차용)
- **장점:** 작은 phase 단순 (PRD·NFR 등 한 파일이 자연), 큰 phase scale (W-{n}·ADR-{n}별 독립 — git diff 친화), v3 차용 (R7 일관)
- **단점:** 두 layout 학습 부담 — 단 v3 README 명시
...
##### 긍정
- v3 README §174-196 그대로 — 도메인 무관성 (R7) 일관
...
|---|---|---|---|---|---|---|---|
| RISK-1 | A1 미충족 — Claude Code SDK가 13 skill state machine orchestration 미지원 | High | High | H×H | maintainer | Phase 13 M0 spike 결과 | ADR-8 spike Phase 13 M0 task. 미통과 시 ADR-6b (hybrid) 설계. v4.0 release 지연 가능 |
| RISK-2 | A2 미충족 — fresh subagent 패턴이 spec phase에서 quality 미보장 | High | High | H×H | maintainer | dogfood (이번 v4 작업)에서 stress test, A1 spike와 함께 | 이번 작업 자체 검증. 미통과 시 R8 scope 축소 (Phase 13 후 implementation은 사용자 수동) |
...
| RISK-10 | NFR-SCAL-2 5000 ID 한계 — 매우 큰 project에서 성능 저하 | Low | Medium | L×M | maintainer | telemetry: graph 크기 추세 | per-project archive 정책 (v4.5 cycle), Phase 11 OPS-15 storage alert |

### Risk 처리 정책

- **High×High (RISK-1, RISK-2):** 즉시 mitigation — Phase 13 M0 spike. v4.0 release 전 cleared.
...
### Non-blocking (배경 결정 — v4.0 출시 후 또는 별 cycle)

| ID | 질문 | Source phase | 결정자 | 마감 (soft) | 상태 |
|---|---|---|---|---|---|
| OQ-1-1 / OQ-11-2 | Marketplace 등록 — v4.0 동시 vs v4.1 안정 후 | Phase 1·11 | maintainer | v4.0 release 전 | OPEN — RB-7 fallback 준비 |
| OQ-1-3 | CC 외 사용자 fallback 가이드 (v3 markdown) — README 어디 | Phase 1 | maintainer | v4.0 release 전 | OPEN |
| OQ-1-4 | telemetry opt-in mechanism (KPI-5·6 측정) | Phase 1 | maintainer | Phase 13 (M2) | Resolved partially — ADR-7로 endpoint, F13.1로 install opt-in. KPI-5 (dashboard) v4.5로 이관 |
| OQ-1-5 / OQ-2-3 | Refactor (S3, P1) v4.1 또는 v4.2 | Phase 1·2 | maintainer | v4.0 release 후 | OPEN — v4.0 telemetry 결과 보고 결정 |
| OQ-2-1 | Edge-1 (non-developer) primary 승격 | Phase 2 | maintainer | v5+ Phase 1 revisit | OPEN (defer) |
| OQ-2-2 / OQ-1-2 | Multi-project tab — v4 또는 v4.1 | Phase 1·2 | maintainer | v4.5 cycle | DEFERRED to v4.5 |
| OQ-3-1 | EXPANSION 후보 e1-e6 cherry-pick | Phase 3 | maintainer | Phase 4 진입 전 | RESOLVED — e6 (R13) 채택, 나머지 deferred |
| OQ-3-2 | F3.5 (frontmatter editing UI) P2 vs P1 | Phase 3 | maintainer | v4.5 | DEFERRED to v4.5 (dashboard) |
...
| OQ-5-4 | N-074 opt-out 명령 surface | Phase 5 | maintainer | v4.5 dashboard | DEFERRED to v4.5 (dashboard UI) |
| OQ-6-1 | `/plan-pipeline find` 명령 v4 포함 vs v4.5 | Phase 6 | maintainer | v4.5 | DEFERRED to v4.5 |
| OQ-6-2 | Settings page (telemetry consent toggle) — CC 명령 vs v4.5 dashboard | Phase 6 | maintainer | v4.5 | DEFERRED — v4.0은 CC 명령만 |
| OQ-7-1 | Z3 출력 형식 — markdown 그대로 vs syntax-highlighted | Phase 7 | maintainer | v4.0 release 후 | OPEN — CC 측 capability 의존 |
| OQ-7-2 | E-CC-7 file path terminal hyperlink — CC 지원 여부 | Phase 7 | maintainer | A1 spike (M0) | OPEN |
| OQ-9-3 | NFR-SCAL-2 5000 ID — per-project archive 정책 | Phase 9 | maintainer | v4.5 | DEFERRED to v4.5 |
| OQ-10-3 | Perf test data 큰 project (5000 ID) 어떻게 생성 | Phase 10 | maintainer | Phase 13 (M3) | OPEN |
| OQ-10-4 | TC-50 telemetry 기반 hook bypass detection 정밀도 | Phase 10 | maintainer | v4.0 release 후 1개월 | OPEN — RISK-3 monitoring 결과 의존 |
| OQ-11-3 | Survey mechanism (KPI-3 self-report) — Google Form / Tally / GitHub issue | Phase 11 | maintainer | v4.0 release 전 | OPEN — Boring choice (GitHub issue template) 권장 |
...
| ID auto-gen (ADR-5) | Sequential counter | v3 차용, 사람 가독, grep 친화 |
| Telemetry endpoint (ADR-7) | Plausible cloud (EU) | Privacy-first 평판, GDPR 자동, $20/월 |
| Phase 산출물 layout (ADR-10) | Single + directory mixed (v3 패턴) | v3 README §174-196 차용 |

**Token 사용 패턴:** 3개 모두 "PAIN 또는 R 핵심 가치 직결"로 token 가치 정당화. 나머지는 표준 도구 + v3 차용. 이 분배는 Cognitive Pattern "Boring by default + 약 3 innovation token" 충실 적용.
...
- M4 — v4.5+ deferred (dashboard, multi-project, refactor S3)
