...
(Local web dashboard는 별 cycle. 본 plugin의 산출물 frontmatter는 미래 dashboard read 호환 design.)

## 2. 문제 (Status Quo)

### 2.1 현재 상황 — 기존 markdown 사용자

Builder가 메인 prompt를 LLM에 paste → 산출물 markdown 작성 → self-check grep 수동 실행 → 다음 phase로. 13번 반복. 검토는 14개 markdown 파일 cross-reference하며 수동.
...
- 기존 markdown 접근의 위 약점 직접 경험 (이번 conversation)
...
- **나이대:** 25-50
- **역할 / 상황:** solo founder · small team · advanced student · AI 도구 자주 쓰는 professional. **추가 조건: Claude Code subscriber 또는 사용자.**
...
- **S3 (Refactor — P1):** 기존 codebase + spec 부재 → plugin이 codebase 분석 후 13 phase 역방향 채움. 초기 release scope X. 향후 후보.
...
- **vs 기존 markdown:** structured I/O로 환각 ID 0건. hook이 self-check 자동. PAIN-4 검토 부담은 markdown rendered (GitHub·VS Code preview)로 부분 mitigate, 인터랙티브 검토는 dashboard cycle에서. (단점: Claude Code 사용자만)
...
기존 접근: "사용자 양심 + LLM 자율" → 약점은 양심·기억·grep 실행 의존.

핵심 통찰: **Spec discipline은 도구로 강제할 때 진짜 작동.** 사용자가 잊어도, LLM이 무시해도, 도구가 차단. 양심·기억은 fail-prone, 도구는 deterministic.

Conventional 접근 (PRD template Notion에 채움): 사용자가 template 채울 의지 있어야. specrail: 채워야만 다음 진행 가능 (HARD-GATE 도구로 enforce).
...
- ❌ **Claude Code 외 다른 agent (Cursor·Aider 등) tooling** — 기존 markdown으로 fallback
...
| KPI-7 | M-CSA attrs block coverage on dogfood spec | % | 100 (post-migration target) · ≥80 (in-progress baseline) — measured by `specrail audit` | per phase status=Approved transition |

(KPI-5 dashboard 사용 빈도 — dashboard cycle로 이동.)
...
<!-- specrail:attrs id=KPI-6 -->
```yaml
target: ">85"
unit: percent
measure-when: "hook log"
linked-r: [R2]
status: Approved
last-modified: 2026-05-16
```
...
linked-r: [R-CSA]
status: Approved
note: "M-CSA dogfood attrs coverage; 100 percent post-migration on maintainer spec, 80 percent secondary on external in-progress spec."
last-modified: 2026-05-16
```
<!-- /specrail:attrs -->
...
| A2 | fresh subagent 패턴이 spec phase에 작동 (산출물 self-sufficiency 강제) | dogfood (이번 작업 자체) | High |
...
(A3 dashboard 관련 가정 — dashboard cycle로 이동.)
...
| OQ-1-2 | 대시보드 multi-project tab (한 brower에 여러 spec) | maintainer | Phase 6 IA | N |
| OQ-1-3 | Claude Code 외 사용자 fallback 가이드 — README 어디 | maintainer | Phase 6 | N |
| OQ-1-4 | telemetry opt-in mechanism (KPI-5·6 측정) | maintainer | Phase 11 | N |
| OQ-1-5 | Refactor (S3, P1) — 향후 cycle 일정 | maintainer | Phase 13 | N |
...
Dashboard·다중 LLM·AI auto-review 등은 별 cycle.
...
- §3.1 Primary Persona (Claude Code 사용자)
