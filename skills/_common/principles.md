...
## Attrs Blocks Are Mandatory (M-CSA, schema v1.0 onwards)

모든 first-class entity (R·F·S·ENT·INV·NFR·ARCH·EXT·OPS·ADR·RISK·TC·EDGE·OQ·KPI·T·PERSONA·SCEN·JNY·ZN·P-CC)는 정의 heading **직후** `<!-- specrail:attrs id=X -->` + fenced YAML + `<!-- /specrail:attrs -->` block을 갖는다.

### 형식 (proposal §3.1, schemas/attrs.schema.json)
...
### Required field per kind

per-entity required fields는 `schemas/attrs.schema.json` $defs/<kind>에 명시. 누락 시 `specrail audit`이 KPI-7 coverage % 감소로 보고, `specrail check`가 `attrs-completeness` finding emit.

### Lifecycle

| plugin version | missing required field | review-required marker |
|---|---|---|
| v0.2.0~v0.4.x | WARN (transition 허용, `specrail migrate --fix` 제안) | ERROR (항상) |
| v0.5.0+ | ERROR (Approved 전환 reject) | ERROR (항상) |

### Typed edges (closed enum 8, ADR-16)

YAML field (plural-noun, 작성 시) ↔ edge kind (singular-verb, 그래프 시) 매핑:

| YAML field | edge kind | source | target |
|---|---|---|---|
| `solves-pains` | `solves` | R/F/S | PAIN |
| `linked-features` | `linked-features` | R | F |
| `parent-r` / `parent-f` / `parent-t` | `parent` | F/S/T | R/F/T |
| `linked-tests` | `tested-by` | R/F/S/AC/INV/NFR | TC/EDGE |
| `linked-ac` | `covers-ac` | TC | AC |
| `mitigates-risks` | `mitigates` | OPS/ARCH/RB | RISK/NFR |
| `linked-arch` | `linked-arch` | R/F/NFR/EXT | ARCH |
| `depends-on` | `depends-on` | T/ARCH/OPS | T/ARCH/OPS/EXT |

Unknown kind = `validateEdgeKind` ERROR. 추가는 minor schema-version bump + 신규 ADR.

### Scalar-metadata fields (typed edge 아님)

`linked-r` (ENT·NFR·OPS), `state-machine` (ENT), `linked-ext` (ARCH·NFR·OPS), `linked-nfr` (TC·OPS) 등은 scalar metadata — `buildTypedEdges`가 무시. 단순 reverse-index 용도.

### Heading-immediate invariant (OQ-CSA-1)

heading line N → attrs marker line N+1 (또는 blank-line 1줄 뒤 N+2). prose가 heading-attrs 사이에 있으면 `attrs-placement` ERROR.

### Review-required marker (OQ-CSA-10)

`<!-- specrail:attrs-review-required reason="..." -->` 가 있으면 `specrail check` 항상 ERROR (version 무관). 해소: payload 수정 후 marker 제거 또는 `specrail audit --accept-codemod-conflict`.
