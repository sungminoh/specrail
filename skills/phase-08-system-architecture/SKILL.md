---
name: phase-08-system-architecture
description: Phase 08 — System Architecture
trigger-words: system-architecture
phase: 08
inputs-from: phase-07
state-machine: explicit (ADR-8 — INV-3 transition gate enforced)
applies-to: 00-common (auto-inject)
---

# Phase 08: System Architecture

이 skill의 본문은 v4 plugin이 `docs/spec/08-system-architecture.md`를 reference link로 inject.

(Architect 옵션 B: T2.5b reference link 방식. v3 수동 instruction은 T2.5c에서 v4 plugin 자동 강제와 정합하도록 주석화됨.)

Output 산출물 경로: `docs/spec/examples/08-system-architecture.md` (실 사용 시 `docs/spec/08-system-architecture.md`로 작성, examples/는 v4 self-application 참고만)
