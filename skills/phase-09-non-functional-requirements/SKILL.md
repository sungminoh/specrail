---
name: phase-09-non-functional-requirements
description: 7 domains - Performance, Scalability, Availability, Security (STRIDE), Privacy, Accessibility (WCAG), i18n. 모두 측정가능.
trigger-words: non-functional-requirements, NFR, performance, security, accessibility, privacy
phase: 09
...
# Phase 09: Non-Functional Requirements
...
- Phase 1 §5 환경
- (DELTA) `docs/spec/09-non-functional-requirements.md` (기존 버전)
...
```bash
# 측정 단위 없는 NFR
grep -E "^\| NFR-" 09-non-functional-requirements.md | awk -F'|' '{print $4}' | grep -iE "좋|빠르|편하|쉽" && echo "측정값 모호"

# STRIDE 6 카테고리 모두
for cat in Spoofing Tampering Repudiation Disclosure DoS Elevation; do
  grep -q "$cat" 09-non-functional-requirements.md || echo "STRIDE $cat 누락"
done

# WCAG 명시 (UI product)
grep -i "WCAG\|2.1\|AA\|AAA" 09-non-functional-requirements.md

# i18n 언어 명시
grep -E "한국어|영어|Korean|English|언어" 09-non-functional-requirements.md

# NFR ↔ ARCH 매핑 존재
grep "NFR.*ARCH\|매핑" 09-non-functional-requirements.md
```
